...
router.post( // Change from GET to POST for image upload
  '/movies/photo/:id',
  auth.enhance,
  ...
  async (req, res, next) => {
    ...
    try {
      ...
      // Save the image URL in the movie document
      movie.image = `${url}/${file.path}`;
      await movie.save();
      
      res.send({ movie, file }); // Respond with the updated movie and file details

    } catch (e) {
      console.log(e);
      res.status(400).send({ error: e.message || 'Error saving the movie image' }); // Return a meaningful error response
    }
  }
);
