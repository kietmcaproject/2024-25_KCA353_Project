...
@app.route('/create_health_rule', methods=['POST'])
def create_health_rule():
    ...
    try:
        health_rule = request.get_json()
        
        # Example validation: Check required fields
        required_keys = {"name", "enabled", "useDataFromLastNMinutes", "affects", "evalCriterias"}
        if not required_keys.issubset(health_rule.keys()):
            return {"error": f"Missing required keys: {required_keys - health_rule.keys()}"}, 400

        # Process the health rule (e.g., store in database, forward to another service)
        logger.info(f"Received health rule: {health_rule}")
        
        # Simulate success response
        return {"message": "Health rule created successfully!", "rule": health_rule}, 201

    except Exception as e:
        logger.error(f"Error processing health rule: {e}")
    ...
    if request.method == 'POST':
        ...
        with get_db_cursor() as (db, cursor):
            ...
            if user and check_password_hash(user['password'], password):
                ...
                session['username'] = user['username']
                session['role'] = user['role']
                session['full_name'] = user['full_name']  # Add this line
                
                flash(f"Welcome {user['full_name']}!", "success")  # Add welcome message
                return redirect(url_for('admin_dashboard' if user['role'] == 'admin' else 'home'))
            else:
                flash("Invalid username or password", "danger")
